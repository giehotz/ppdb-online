...
<div class="px-4 md:px-8 lg:px-12 py-6">
    ...
    <div class="bg-gradient-to-r from-gray-50 to-gray-100 rounded-2xl p-8 mb-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <a href="#" id="toggle-registration" class="bg-white hover:bg-gray-50 rounded-xl p-4 text-center transition-all duration-200 transform hover:scale-105 shadow-md">
                <i class="fas fa-toggle-on text-2xl text-indigo-500 mb-2"></i>
                <p class="text-sm font-medium text-gray-700">Toggle Pendaftaran</p>
            </a>
        </div>
    </div>
    
    <script>
    document.getElementById('toggle-registration').addEventListener('click', function(e) {
        e.preventDefault();
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                if (data.is_open) {
                    icon.classList.remove('fa-toggle-off', 'text-red-500');
                    icon.classList.add('fa-toggle-on', 'text-indigo-500');
                } else {
                    icon.classList.remove('fa-toggle-on', 'text-indigo-500');
                    icon.classList.add('fa-toggle-off', 'text-red-500');
                }
            } else {
                alert('Gagal mengubah status pendaftaran: ' + data.message);
            }
        })
        .catch(error => {
